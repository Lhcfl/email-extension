...
module Email
  class Receiver
    def self.email_extension_reply_by_email_address_regex
      reply_addresses = SiteSetting.email_extension_reply_by_email_address.split("|").map do |ad|
        ad = Regexp.escape(ad)
        ad.gsub!("\+", "\+?")
    ...
    def process_destination(destination, user, body, elided)
      ...
      if destination.is_a?(Group)
        ...
        # reply extension
        if (user.nil? || user.staged?) && destination.topic.category.is_a?(Category) && !destination.topic.category.email_in_allow_strangers
          raise StrangersNotAllowedError
        end
